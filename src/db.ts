...
export async function restore(uid: string, data: { subjects: Subject[]; attendance: Attendance[]; timetable: TimetableEntry[]; settings: Settings }, replace: boolean) { const db = await dbPromise; const tx = db.transaction(['subjects','attendance','timetable','settings'], 'readwrite'); if (replace) { for (const name of ['subjects','attendance','timetable'] as const) { const store = tx.objectStore(name); const keys = await store.index('by-uid').getAllKeys(uid); await Promise.all(keys.map(k => store.delete(k))); } await tx.objectStore('settings').delete(uid); } for (const s of data.subjects) await tx.objectStore('subjects').put({ ...s, uid }); for (const a of data.attendance) await tx.objectStore('attendance').put({ ...a, uid }); for (const t of data.timetable) await tx.objectStore('timetable').put({ ...t, uid }); await tx.objectStore('settings').put({ ...data.settings, uid }); await tx.done; }
