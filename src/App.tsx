...
function Dashboard({subjects,records,table,stats,onMark,onAdd,onEdit,onNav}:{subjects:Subject[];records:Attendance[];table:TimetableEntry[];stats:ReturnType<typeof overall>;onMark:(id:string,s:AttendanceStatus,date?:string,sessionId?:string)=>void;onAdd:()=>void;onEdit:(s:Subject)=>void;onNav:(p:Page)=>void}) {
  ...
  return <>
    ...
    <TodaysClasses entries={today} subjects={subjects} records={records} currentId={currentClass?.id} nextId={next?.id} onMark={onMark} onSetup={()=>onNav('timetable')}/>
...
function TodaysClasses({entries,subjects,records,currentId,nextId,onMark,onSetup}:{entries:TimetableEntry[];subjects:Subject[];records:Attendance[];currentId?:string;nextId?:string;onMark:(id:string,s:AttendanceStatus,date?:string,sessionId?:string)=>void;onSetup:()=>void}){
  const date=dateISO();
  return <section className="todays-classes">
    <div className="section-title"><div><h2>Today’s classes</h2><p>{entries.length?'One tap marks the class':'Nothing on the timetable for today'}</p></div></div>
    {!entries.length?<Empty title="No classes today" text="Enjoy the day off, or set up your timetable if this looks wrong." action="Set up timetable" onAction={onSetup}/>:<div className="today-list">
      {entries.map(e=>{
        const subject=subjects.find(s=>s.id===e.subjectId);
        const record=records.find(r=>r.subjectId===e.subjectId&&r.date===date&&r.sessionId===e.id);
        const status=record?.status;
        return <article key={e.id} className={`card today-class${e.id===currentId?' now':''}${status==='cancelled'?' is-cancelled':''}`} style={{'--accent':subject?.color??colors[0]} as CSSProperties}>
          ...
          <div className="today-class-info">
            ...
            <p>{e.type} · {e.room||'Room TBA'}{status&&status!=='unmarked'&&<> · <b className={`status-tag-${status}`}>{status[0].toUpperCase()+status.slice(1)}</b></>}</p>
          </div>
          <div className="today-class-actions">
            <button aria-label={`Mark ${e.subject} present`} className={`tc-btn present${status==='present'?' picked':''}`} onClick={()=>onMark(e.subjectId,'present',date,e.id)}><Check/></button>
            <button aria-label={`Mark ${e.subject} absent`} className={`tc-btn absent${status==='absent'?' picked':''}`} onClick={()=>onMark(e.subjectId,'absent',date,e.id)}>−</button>
            <button aria-label={`Mark ${e.subject} cancelled`} className={`tc-btn cancelled${status==='cancelled'?' picked':''}`} onClick={()=>onMark(e.subjectId,'cancelled',date,e.id)}>Cancelled</button>
          </div>
        </article>
      })}
    </div>}
